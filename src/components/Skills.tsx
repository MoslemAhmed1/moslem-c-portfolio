...
const Skills = () => {
  const technicalSkills = [
    { name: "C++", category: "primary" },
    { name: "Python", category: "primary" },
    { name: "ARM Assembly", category: "specialty" },
    { name: "Embedded C", category: "specialty" },
    { name: "STM32", category: "hardware" },
    { name: "Verilog", category: "hardware" },
    { name: "SQL", category: "secondary" },
  ...
  const getSkillColor = (category: string) => {
    switch (category) {
      case "primary": return "bg-tech text-tech-foreground";
      case "specialty": return "bg-cta text-cta-foreground";
      case "hardware": return "bg-primary text-primary-foreground";
      case "tools": return "bg-secondary text-secondary-foreground";
      case "libraries": return "bg-accent text-accent-foreground";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Combining deep technical expertise with practical problem-solving to deliver efficient embedded solutions
          </p>
        </div>
        ...
        <div className="space-y-16">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {valuePropositions.map((item, index) => (
              <Card key={index} className="p-8 text-center shadow-card border-tech/10 hover:shadow-elevated transition-smooth">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {technicalSkills.filter(skill => ["primary", "specialty"].includes(skill.category)).map((skill, index) => (
                      <Badge key={index} className={getSkillColor(skill.category)}>
                        {skill.name}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {technicalSkills.filter(skill => skill.category === "hardware").map((skill, index) => (
                      <Badge key={index} className={getSkillColor(skill.category)}>
                        {skill.name}
                      </Badge>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {technicalSkills.filter(skill => ["tools", "libraries", "secondary"].includes(skill.category)).map((skill, index) => (
                      <Badge key={index} className={getSkillColor(skill.category)}>
                        {skill.name}
                      </Badge>
                    ))}
            ...
            <Card className="p-8 shadow-card">
              ...
              <div className="space-y-4">
                {softSkills.map((skill, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-smooth">
